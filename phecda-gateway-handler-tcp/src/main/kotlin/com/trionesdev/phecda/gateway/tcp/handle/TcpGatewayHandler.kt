...
import com.trionesdev.phecda.gateway.core.GatewayHandlerProperties
...
@Sharable
abstract class TcpGatewayHandler : ChannelInboundHandlerAdapter(), GatewayHandler {
    @Autowired
    ...
    override fun postProperties(properties: PhecdaEvent) {
        kafkaTemplate?.send(
            gatewayHandlerProperties?.propertiesPostTopic ?: GatewayHandlerProperties.PROPERTIES_POST_TOPIC,
            JSON.toJSONBytes(properties)
        )
    }

    override fun postEvents(properties: PhecdaEvent) {
        kafkaTemplate?.send(gatewayHandlerProperties?.eventsPostTopic ?: GatewayHandlerProperties.EVENTS_POST_TOPIC, JSON.toJSONBytes(properties))
    }
